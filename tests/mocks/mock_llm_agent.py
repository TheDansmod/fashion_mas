...
class MockAgent:
        ...
        if self.schema_name == "UpdatedUserRequest":
            ...
            return {
                "structured_response": self.schema(
                    ...
                    updated_user_query="Mock updated user query.",
                )
            }
        else:
            raise NotImplementedError(
                f"invoking agent for this schema name {self.schema_name} is not implemented"
            )
    ...
    async def ainvoke(self, *args, **kwargs):
        if self.schema_name == "MatchedImageId":
            ...
            raise NotImplementedError(
                f"async invoking agent for this schema name {self.schema_name} is not implemented"
            )
...
class MockStructuredOutput:
    ...
    def invoke(self, *args, **kwargs):
        if self.schema_name == "NumRecommendations":
            ...
            # for the vision node
            return self.schema(
                image_description="A blue cotton shirt with short sleeves."
            )
            ...
            raise NotImplementedError(
                f"structured mock not implemented for {self.schema_name}"
            )
...
class ChatMockLLM:
    def __init__(self, *args, **kwargs):
        ...
        self.intent_node_return = AIMessage(
            content="Mock response: Focus on the stylistic coherence of the lower body garments."
        )
        ...
        self.explanation_node_return = AIMessage(
            content="The properties of the input image synergize with the input image"
        )
    ...
    def invoke(self, prompt, *args, **kwargs):
        if isinstance(prompt, str):
            ...
            if self.intent_node_prompt_substring.lower() in prompt_str:
                ...
                raise NotImplementedError(
                    f"Invoking model for this string prompt: {prompt} is not implemented"
                )
            ...
            for content in prompt[0].content:
                ...
                if (
                    content["type"] == "text"
                    and self.explanation_node_prompt_substring.lower()
                    in content["text"].lower()
                ):
                    return self.explanation_node_return
            raise NotImplementedError(
                f"Mock LLM in list prompt could not find the right text: {prompt}"
            )
        else:
            raise NotImplementedError(
                f"Mock LLM got input that was neither list nor string: {prompt}"
            )
