...
class EmbeddingProviderSettings(BaseSettings):
    ...
    # danish: meaning you can create a user by doing User(username='John') if username is a validation alias for name attribute in the class. But while printing or passing the object, only the actual attribute name is used
    # danish: the other option is a serialization alias which can be used during printing.
    # danish: if the aliases are combined - you want to use the same one for validation and serialization then you can set alias='something'
