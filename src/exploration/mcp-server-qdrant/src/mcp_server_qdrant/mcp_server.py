...
class QdrantMCPServer(FastMCP):
    ...
    def setup_tools(self):
        ...
        async def store(
            ...
            # danish: Metadata is just a dict[str, Any] used for the actual qrant point metadata
            metadata: Annotated[
                Metadata | None,
                Field(
        ...
        if len(filterable_conditions) > 0:
            # danish: the below function uses the filterable fields, removes the generic query in the find function, and replaces it with fields with their annotations and descriptions, that the MCP and LLMs can see - like 'color' and 'size'
            find_foo = wrap_filters(find_foo, filterable_conditions)
        elif not self.qdrant_settings.allow_arbitrary_filter:
            # danish: make_partial_function: this function pre-fills specific arguments similar to functool.partial, but by modifying the signature of the function it then also ensures that tools like FastAPI which might read the signature of a function, don't see those fixed / filled in values in the signature
