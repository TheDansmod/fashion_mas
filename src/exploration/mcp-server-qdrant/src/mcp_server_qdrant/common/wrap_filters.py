...
# danish: this function re-writes the original function which accepts a generic filter object ({'query_filter': None}) into a new function that accepts specific explicitly named fields (color=red, size=20). it also updates the parameters of the original function and adds annotations so that APIs like FastAPI or MCP protocol can `see` the new parameters, allowing the LLM to figure out the filterable fields.
def wrap_filters(
