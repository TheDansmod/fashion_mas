...
def vision_node_update(update):
    tabbed_descriptions = [
        f"\t- {descr}" for descr in update.get("input_images_descriptions", [])
    ]
...
def modifier_node_update(update):
    tabbed_descriptions = [
        f"\t- {descr}" for descr in update.get("required_clothes_descriptions", [])
    ]
...
NODE_META = {
    "human_node": ("Start Node ❯❯❯❯", lambda u: f"Synthesizing Inputs"),
    "quantifier_node": (
        "Quantifier Node 🔢",
    ...
    "recommender_node": (
        ...
        lambda u: (
            f"Matched **{len(u.get('recommended_clothes_image_paths', []))}** item(s) from the Product Catalogue."
        ),
    ),
    "critique_node": (
        "Critique Node 🤔",
