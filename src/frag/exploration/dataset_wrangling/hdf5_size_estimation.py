# THIS FILE MIGHT BE IN THE WRONG LOCATION
# THIS FILE MIGHT BE IN THE WRONG LOCATION
# THIS FILE MIGHT BE IN THE WRONG LOCATION
# THIS FILE MIGHT BE IN THE WRONG LOCATION
# THIS FILE MIGHT BE IN THE WRONG LOCATION
# THIS FILE MIGHT BE IN THE WRONG LOCATION
# I moved it into the dataset_wrangling folder
import h5py
import numpy as np
...
def get_sizes():
    ...
    meta_disk = sum(
        v["storage_size"] for k, v in results.items() if k not in IMAGE_KEYS
    )
    ...
    for k, v in results.items():
        ...
        print(
            f"[{label}] {k:<30} | shape: {str(v['shape']):<25} | "
...
def create_hdf5_file(
    output_path: str, image_paths: list[str], descriptions: list[str]
) -> None:
    """
    ...
    images = np.stack(
        [np.array(Image.open(p).convert("RGB")) for p in image_paths], axis=0
