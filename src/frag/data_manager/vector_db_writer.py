...
class FashionSigLIPEmbedding:
    ...
    def __init__(
        ...
        embedding_batch_size: str = PV[cfg.data.vector_db.embedding_batch_size],
    ):
...
def get_vector_db_client(
    ...
    recreate: bool = PV[cfg.data.vector_db.recreate],
    collection_name: str = PV[cfg.data.vector_db.collection_name],
    image_vectors_name: str = PV[cfg.data.vector_db.image_vectors_name],
    text_vectors_name: str = PV[cfg.data.vector_db.text_vectors_name],
    embedding_size: int = PV[cfg.data.data_processing.embedding_size],
    vectors_on_disk: bool = PV[cfg.data.vector_db.vectors_on_disk],
    on_disk_payload: bool = PV[cfg.data.vector_db.payload_on_disk],
    indexing_threshold: int = PV[cfg.data.vector_db.indexing_threshold],
    hnsw_on_disk: bool = PV[cfg.data.vector_db.hnsw_on_disk],
):
...
def _iter_data_points(
    ...
    text_vec_name: str = PV[cfg.data.vector_db.text_vectors_name],
    images_key: str = PV[cfg.data.fashion_gen.images_key],
    descriptions_key: str = PV[cfg.data.fashion_gen.descriptions_key],
    prices_key: str = PV[cfg.data.fashion_gen.prices_key],
    index_key: str = PV[cfg.data.fashion_gen.index_key],
    num_points: int = PV[cfg.data.fashion_gen.num_datapoints],
    fetch_batch_sz: int = PV[cfg.data.data_processing.data_fetch_batch_size],
    string_attributes: list[str] = PV[cfg.data.fashion_gen.string_attributes],
):

    for from_idx in range(0, num_points, fetch_batch_sz):
        to_idx = min(from_idx + fetch_batch_sz, num_points)
        data = get_fashion_gen_data(from_idx, to_idx)
        ...
        for idx, (img_vec, text_vec) in enumerate(img_descr_pairs):
            ...
            string_payload = {key: data[key][idx] for key in string_attributes}
            payload = {
...
def populate_vector_db(
    ...
    upload_parallel: int = PV[cfg.data.data_processing.upload_parallel],
):
    ...
    client.upload_points(
        collection_name=collection_name,
        points=_iter_data_points(embedder),
