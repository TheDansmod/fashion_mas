        ...
        for query_idx, input_text in enumerate(group_queries):
            ...
            if image_name in input_images:
                ...
                query_input = {
                    ...
                    "query_num_in_group": query_idx + 1,
                }
            else:
                query_input = {
                    "group_num": group_idx + 1,
...
async def run_fashion_agent_single_input(eval_input, callback_config):
    ...
    try:
        ...
        resume_payload = {
            ...
            "input_images_path": eval_input["input_images_path"],
        }
        result = await agent.ainvoke(Command(resume=resume_payload), config=config)
        log.debug(f"got result {result}")
        if (
            "recommended_clothes_image_paths" in result
            and "recommended_clothes_descriptions" in result
        ):
            recommended_clothes_image_paths = result["recommended_clothes_image_paths"]
            recommended_clothes_descriptions = result[
                "recommended_clothes_descriptions"
            ]
        else:
            log.error(
...
async def run_evaluation_set():
    ...
    for eval_input in get_evaluation_inputs():
        ...
        result = {
            "recommended_clothes_image_paths": paths,
            "recommended_clothes_descriptions": descr,
            **eval_input,
        }
        write_result(result)
...
def judge_evaluation_outputs(
    ...
    for eval_run in data:
        ...
        if (
            len(eval_run["recommended_clothes_image_paths"]) == 0
            or len(eval_run["recommended_clothes_descriptions"]) == 0
        ):
            continue
        # generate the prompt
        has_input_image = len(eval_run["input_images_path"]) > 0
        prompts = []
        for img_path in eval_run[
            "input_images_path"
        ]:  # we want error if the key is not in the dict
            prompts.append(("image", img_path))
        prompt_prefix = (
            "All the above are user input images.\n\n" if has_input_image else ""
        )
        prompts.append(
            (
                "text",
                f"{prompt_prefix}The user request is: {eval_run['input_text']}\n\n",
            )
        )
        ...
        try:
            ...
            log.exception(
                "For group: {eval_run['group_num']}, query: {eval_run['query_num_in_group']} There was an exception while invoking the structured model for judgement."
            )
            continue
        log.debug(
            f"For group: {eval_run['group_num']}, query: {eval_run['query_num_in_group']} Got response from model:\n{response}"
        )
        # compute final score
